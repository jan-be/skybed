...
def init_traffic_control(network_id: str):
    ...
    add_root_qdisc_command = f"tc qdisc add dev {interface} root handle 1: htb default 1 r2q 1"
...
def slow_down_container_network(container: UAVContainer, network_params: NetworkParams):
    ...
    try:
        if not container.tc_class_added:
            add_class_command = f"tc class add dev {interface} parent 1: classid 1:{container.id[:4]} htb rate 1000mbit quantum 1514"
            subprocess.run(shlex.split(add_class_command), check=True, capture_output=True, text=True)

        netem_command = f"tc qdisc {"change" if container.tc_class_added else "add"} dev {interface} parent 1:{container.id[:4]} handle {container.id[4:8]}: netem delay {network_params.delay}ms rate {network_params.throughput}mbit loss {network_params.packet_loss * 100}%"
        subprocess.run(shlex.split(netem_command), check=True, capture_output=True, text=True)

        if not container.tc_class_added:
            add_class_command = f"tc filter add dev {interface} protocol ip parent 1:0 prio 3 u32 match ip dst {container.throttled_ip} flowid 1:{container.id[:4]}"
            subprocess.run(shlex.split(add_class_command), check=True, capture_output=True, text=True)
